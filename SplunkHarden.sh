...
EOF
  ...
  # web.conf - HTTPS + disable XSLT vulnerability
...
EOF
  ...
  # Set permissions on local configs only
...
setup_aide() {
  ...
  # AIDE check script with syslog alerts
...
EOF
  ...
  # Schedule AIDE checks every 15 minutes
...
EOF
  ...
  # Lock only human accounts (UID >= 1000), skip our admins
  while IFS=: read -r username _ uid _ _ _ shell; do
    # Skip root, sysadmin, splunk, and any wheel members
    if [[ "$username" =~ ^(root|sysadmin|splunk)$ ]]; then
      continue
    fi
    
    # Check if user is in wheel group (our admins)
    if groups "$username" 2>/dev/null | grep -q '\bwheel\b'; then
      continue
    fi
    
    # Only lock UID >= 1000 (human accounts)
    if [[ "$uid" -ge 1000 ]]; then
...
cronjail() {
  ...
  # Copy (not move) existing cron jobs for review
...
secure_cron_at() {
  ...
  # Cron security - use allow file, don't contradict with deny
  touch /etc/cron.allow
  chmod 600 /etc/cron.allow
  
  # Add admin users to allow
  echo "root" > /etc/cron.allow
  id -u sysadmin >/dev/null 2>&1 && echo "sysadmin" >> /etc/cron.allow
  
  # Clear deny file (allow takes precedence)
  : > /etc/cron.deny
...
main() {
  ...
  # Cron security (jail first, then secure)
  cronjail
  secure_cron_at
